...
if (!$user || !user_has_any_role(['marketer'])) {
    add_flash('danger', 'Please login as a marketer to edit listings.');
...
if (!$property || !can_user_edit_property($property, $user)) {
    ...
    header('Location: /DigiHome/marketer/listings.php');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['delete_existing_image'])) {
        ...
        header('Location: /DigiHome/marketer/edit-property.php?id=' . $propertyId);
        exit;
    }
...
$pageTitle = 'DigiHome | Edit Assigned Property';
$pageDescription = 'Update assigned listing details based on your permission settings.';
...
<section class="section-shell" data-reveal>
    <div class="section-head">
        <div>
            <h2>Edit Property</h2>
            <p>You can edit only listings allowed under your marketer permissions.</p>
        </div>
    </div>
    <form method="post" class="form-grid is-3" enctype="multipart/form-data">
            ...
            <div class="property-media-grid">
                <?php foreach ((array) ($property['images'] ?? []) as $imageIndex => $imageUrl): ?>
                    ...
                    <article class="property-media-item">
                        ...
                        <button type="submit" class="ghost-button" name="delete_existing_image" value="<?= (int) $imageIndex ?>" formaction="/DigiHome/marketer/edit-property.php?id=<?= (int) $propertyId ?>" formmethod="post" onclick="return confirm('Delete this image now? This updates the database immediately.');">Delete image</button>
                    </article>
                <?php endforeach; ?>
            </div>
        ...
        <div class="inline-actions full-span">
            ...
            <a class="ghost-button" href="/DigiHome/marketer/listings.php">Cancel</a>
        </div>
    </form>
</section>
