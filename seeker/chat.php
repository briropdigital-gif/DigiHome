...
$user = current_user();
$isSeekerLoggedIn = $user && user_has_any_role(['property_seeker']);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'send_message') {
        if (!$isSeekerLoggedIn) {
            add_flash('warning', 'Please login as a property seeker to send a message.');
            header('Location: /DigiHome/seeker/chat.php');
            exit;
        }
        ...
        if ($messageBody !== '') {
            if ($conversationId <= 0) {
                $conversation = create_or_get_open_conversation((int) $user['id'], $user['role'], 'General Support');
    ...
    if ($action === 'reopen_conversation') {
        if (!$isSeekerLoggedIn) {
            add_flash('warning', 'Please login to manage a conversation.');
            header('Location: /DigiHome/seeker/chat.php');
            exit;
        }
        $conversationId = (int) ($_POST['conversation_id'] ?? 0);
        if ($conversationId > 0 && set_conversation_status($conversationId, 'open')) {
            add_flash('success', 'Conversation reopened.');
        }
    }
...
$conversations = $isSeekerLoggedIn ? get_user_conversations((int) $user['id'], $user['role']) : [];
...
$chatAccountHref = '/DigiHome/includes/account.php?mode=login&return=' . urlencode('/DigiHome/seeker/chat.php');

function chat_tick_for_user_message(array $message): string {
...
$pageTitle = 'DigiHome | Support Chat';
$pageDescription = 'Chat with DigiHome support and track conversation status.';
...
<section class="section-shell" data-reveal>
    ...
    <div class="chat-shell" data-chat-app data-chat-role="<?= htmlspecialchars((string) ($user['role'] ?? 'property_seeker')) ?>" data-chat-user-id="<?= (int) ($user['id'] ?? 0) ?>" data-chat-state-url="/DigiHome/includes/chat-api.php" data-chat-auth-token="<?= htmlspecialchars($chatAuthToken) ?>">
        <aside class="chat-list" data-chat-list>
            ...
            <?php else: ?>
                <?php foreach ($conversations as $conversation): ?>
                    ...
                    <a class="chat-item-link" href="/DigiHome/seeker/chat.php?id=<?= (int) $conversation['id'] ?>" data-chat-conversation-link data-conversation-id="<?= (int) $conversation['id'] ?>">
        ...
        <div class="chat-main">
            <?php if ($activeConversation): ?>
                ...
                <div class="chat-thread" data-chat-thread>
                    <?php foreach ($messages as $message): ?>
                        <?php $isMine = $isSeekerLoggedIn && (int) ($message['sender_user_id'] ?? 0) === (int) $user['id']; ?>
                        <?php $messageAttachments = is_array($message['attachments'] ?? null) ? $message['attachments'] : []; ?>
                        <div class="chat-bubble <?= $isMine ? 'mine' : 'other' ?>" data-message-id="<?= (int) ($message['id'] ?? 0) ?>">
                            ...
                            <div class="chat-meta">
                                ...
                                <?php if ($isMine): ?>
                                    <?php $tick = chat_tick_for_user_message($message); ?>
                                    <span class="chat-meta-actions">
                                        ...
                                        <span class="chat-tick <?= !empty($message['read_at']) ? 'is-read' : (!empty($message['delivered_at']) ? 'is-delivered' : 'is-sent') ?>" data-chat-tick><?= $tick ?></span>
                                    </span>
                                <?php endif; ?>
                            </div>
                        </div>
            ...
            <?php if ($isSeekerLoggedIn): ?>
                <form method="post" class="chat-compose" data-chat-compose enctype="multipart/form-data" novalidate>
                    ...
                    <textarea name="message_body" placeholder="Type your message to admin..." data-chat-message-input></textarea>
