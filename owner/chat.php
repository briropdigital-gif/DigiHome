...
$user = current_user();
$isOwnerLoggedIn = $user && user_has_any_role(['property_owner']);

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!$isOwnerLoggedIn) {
        add_flash('warning', 'Please login as a property owner to send a message.');
        header('Location: /DigiHome/owner/chat.php');
        exit;
    }
    ...
    if ($messageBody !== '') {
        if ($conversationId <= 0) {
            $conversation = create_or_get_open_conversation((int) $user['id'], $user['role'], 'Owner Support');
...
$conversations = $isOwnerLoggedIn ? get_user_conversations((int) $user['id'], $user['role']) : [];
...
$chatAccountHref = '/DigiHome/includes/account.php?mode=login&return=' . urlencode('/DigiHome/owner/chat.php');
$pageTitle = 'DigiHome | Owner Support Chat';
...
<section class="section-shell" data-reveal>
    ...
    <?php endif; ?>
    <div class="chat-shell" data-chat-app data-chat-role="<?= htmlspecialchars((string) ($user['role'] ?? 'property_owner')) ?>" data-chat-user-id="<?= (int) ($user['id'] ?? 0) ?>" data-chat-state-url="/DigiHome/includes/chat-api.php" data-chat-auth-token="<?= htmlspecialchars($chatAuthToken) ?>">
        <aside class="chat-list" data-chat-list>
            ...
            <?php else: ?>
                <?php foreach ($conversations as $conversation): ?>
                    ...
                    <a class="chat-item-link" href="/DigiHome/owner/chat.php?id=<?= (int) $conversation['id'] ?>" data-chat-conversation-link data-conversation-id="<?= (int) $conversation['id'] ?>">
            ...
            <?php if ($activeConversation): ?>
                ...
                <div class="chat-thread" data-chat-thread>
                    <?php foreach ($messages as $message): ?>
                        <?php $mine = $isOwnerLoggedIn && (int) ($message['sender_user_id'] ?? 0) === (int) $user['id']; ?>
